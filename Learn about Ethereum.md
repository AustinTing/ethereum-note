...
- 所有交易發動前都要符合以下條件：
  - 符合RLP格式，一種Ethereum序列化物件的格式。
  - 合法的交易簽名。
  - 合法的交易nonce。也就是之前說的帳戶發送交易的次數。
  - 交易gas limit必須大於intrinsic gas。（根據這次交易推算出來的基礎花費）
  - 發送方的帳戶餘額必須大於upfront gas costs。（這次發送的Ether加上gas limit所需Ether）。
- 滿足條件後，依照下面步驟執行交易：
  - 發送方帳戶扣除upfront gas costs。
  - 發送方帳戶nonce 加1。
  - 交易正式執行，過程中會透過子狀態紀錄當交易完成時，立刻所需要的資訊。子狀態包含：
    - Self-destruct set: 一組會被拋棄的帳戶（如果有的話）。
    - Log series。
    - Refund balance: 交易完成後退回發送方帳戶的金額。之前提到存東西進Ethereum會有成本，如果清除資料則可以獲得獎勵。
  - 交易中的各種運算。
  - 如處理交易的過程中沒發生問題，refund balance會被退回發送方的帳戶。
  - 發送方獲得refund balance後：
    - 礦工獲得瓦斯。
    - gas的教耗加入至區塊。
    - Self-destruct set中的帳戶會被刪除（如果有的話）。
  - 最後，交易創造了新的狀態和log。
- 合約創建和訊息呼叫有些不同，下面會作說明。

##### 合約創建

利用特殊的合約公式宣告address後，經由以下步驟初始化合約帳戶。
- 把nonce設成零
- 如果發送方要發送Ether進此合約帳戶，調整此合約帳戶餘額。
- 把發送方的帳戶餘額扣除發送給合約帳戶的Ether。
- 將storage設定成空。
- 將合約的codeHash設定成空字串。
- 初始化完後，利用交易夾帶init code去創建真正的帳戶。
- 根據init code，執行相關動作。
- 執行初始化程式會消耗gas，一旦gas消耗完但還沒完成，會出發out-of-gas (OOG) 例外。交易會還原到交易前的狀態。且gas不會被返回給發送方。
  - 但是，發送方要發送給合約帳戶的Ether會被返環。
- 初始化程式成功後，要付一筆最終的合約創建費用。
  - 合約的儲存成本，跟合約的程式碼大小成比例。
  - 如果剩下的瓦斯不夠，此次創建合約交易還是一樣會出發out-of-gas (OOG) 然後被取消。
- 順利完成，多的gas還是會還給發送方，狀態更新。

##### 訊息呼叫

- 訊息呼叫跟合約創建只有幾點不同。
  - 訊息呼叫沒有init code。
  - 訊息呼叫可以包含input data，如果交易發送方有提供的話。
- 在過去，訊息呼叫有發生錯誤或OOG時，未使用到或剩餘的gas不會被退回。但在最近的更新，這些gas會被返還給使用者。

#### 執行模型

Protocol中，真正處理交易的是Ethereum自己的虛擬機，又稱Ethereum Virtual Machine (EVM)。

- EVM跟典型圖靈完備機就差在EVM可執行計算的總和被gas所限制。
- EVM是一種基於stack的架構，後進先出。
- EVM有自己的語言「EVM bytecode」。開發者用像是Solidity的高階語言寫合約，寫完後編譯成EVM bytecode讓EVM看得懂。

##### 一個區塊完成了
- 已完成（finalized）有兩個意思
    1. 如果是新區塊，是指針對挖這個區塊的所有過程。
    2. 如果是現有區塊，是指針對區塊的驗證。
#### 挖礦的工作證明（Proof of Work, PoW）

一個賦予區塊難度意義的演算法。

- Ethereum的PoW稱作「Ethash」。
##### 設計挖礦的兩個目的
- 做為一種安全機制
  - 除了枚舉法沒有更好的方法可以找到低於所需要閥值的隨機數。
  - 讓駭客很難在鏈上去修改其中一個區塊，或是維護一個分叉，除非他的算力要比所有眾人還多。
- 做為一種財富分配的機制
  - 藉由提供算力讓系統更安全來獲得系統分配的財富。
